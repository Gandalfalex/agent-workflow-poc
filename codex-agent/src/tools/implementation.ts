...
export async function implementTicket(
  ...
  try {
    ...
    // Step 8: Deploy as shadow service (if implementation succeeded)
    let shadowDeploymentInfo = "";
    if (subagentResult.success) {
      console.error("[Implement] Step 7: Setting up shadow deployment...");
      const shadowInfo = await setupShadowDeployment(
        ticket.key,
        branch,
        worktreeInfo.repoRoot,
      );
      if (shadowInfo.success) {
        shadowDeploymentInfo = shadowInfo.instructions;
      } else {
        console.error(
          `[Implement] Warning: Shadow deployment setup failed: ${shadowInfo.error}`,
        );
      }
    }
    ...
    const output: ImplementationOutput = {
      ...
      nextSteps: shadowDeploymentInfo
        ? [...(subagentResult.nextSteps || []), shadowDeploymentInfo]
        : subagentResult.nextSteps,
...
/**
 * Setup shadow deployment for the feature branch
 * Creates docker-compose configuration and deployment instructions
 */
async function setupShadowDeployment(
  ticketKey: string,
  branch: string,
  repoRoot: string,
): Promise<{
  ...
  try {
    const workspaceName = branch.replace(/[^a-z0-9-]/gi, "-").toLowerCase();
    const worktreePath = path.join(repoRoot, "worktrees", workspaceName);
    const composeFilePath = path.join(repoRoot, "docker-compose.prod.yaml");
    ...
    if (!fs.existsSync(composeFilePath)) {
      return {
        ...
        instructions: "",
        error:
          "docker-compose.prod.yaml not found - skipping shadow deployment setup",
      };
    }

    // Generate docker-compose snippet for shadow service
    const serviceName = `ticketing-api-shadow-${workspaceName}`;
    const dbName = `ticketing-postgres-shadow-${workspaceName}`;

    const shadowServiceYaml = `
  # Shadow deployment for ${ticketKey} (${branch})
  ${serviceName}:
    build:
      context: ./worktrees/${workspaceName}/ticketing-system
      dockerfile: backend/Dockerfile
    environment:
      PORT: "8080"
      DATABASE_URL: postgres://ticketing:ticketing@${dbName}:5432/ticketing?sslmode=disable
...
volumes:
  ${serviceName}-data:`;
    ...
    const instructions = `🚀 **Shadow Deployment Ready**

To deploy this feature as a shadow service for comparison testing:

1. **Add to docker-compose.prod.yaml:**
   Copy these services to the end of docker-compose.prod.yaml (before networks/volumes):
   \`\`\`yaml
${shadowServiceYaml}
   \`\`\`
...
2. **Build and deploy:**
   \`\`\`bash
   cd ${repoRoot}
   docker compose -f docker-compose.prod.yaml build ${serviceName}
   docker compose -f docker-compose.prod.yaml up -d ${serviceName}
   \`\`\`

3. **Monitor the shadow service:**
   \`\`\`bash
   docker compose -f docker-compose.prod.yaml logs -f ${serviceName}
   \`\`\`

4. **Configure mirroring in Shadowing Admin UI:**
   - Access: http://localhost:8081
   - Create a new route with:
     - **Primary:** http://ticketing-api-prod:8080
     - **Shadow:** http://${serviceName}:8080
   - Start making requests to mirror traffic

5. **Review comparison results:**
   - View side-by-side responses in the Shadowing Admin UI
   - Compare status codes, headers, and response bodies
   - Check latency differences

6. **Promote to production (when ready):**
   - Merge branch to main: \`git checkout main && git merge ${branch}\`
   - Rebuild production: \`docker compose -f docker-compose.prod.yaml build ticketing-api-prod\`
   - Restart: \`docker compose -f docker-compose.prod.yaml up -d ticketing-api-prod\`

7. **Clean up shadow service:**
   - Remove from docker-compose.prod.yaml
   - Stop: \`docker compose -f docker-compose.prod.yaml down ${serviceName}\`
   - Cleanup: \`docker volume rm coding-agent-workflow_${serviceName}-data\`
...
**Service:** \`${serviceName}\``;
    ...
    console.error(
      `[Implement] Shadow deployment configuration generated for ${serviceName}`,
    );
    ...
    return {
      success: false,
      instructions: "",
      error: `Failed to setup shadow deployment: ${errorMessage}`,
    };
  }
}
