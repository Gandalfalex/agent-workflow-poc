import { computed, ref } from "vue";

type Locale = "en" | "de";
type TranslationTable = Record<string, string>;

const STORAGE_KEY = "app.locale";

const messages: Record<Locale, TranslationTable> = {
    en: {
        "common.clear": "Clear",
        "common.loading": "Loading...",
        "common.close": "Close",
        "common.cancel": "Cancel",
        "common.delete": "Delete",
        "common.save": "Save",
        "common.add": "Add",
        "board.search.placeholder": "Filter tickets...",
        "board.toolbar.views": "Views",
        "board.toolbar.filters": "Filters",
        "board.toolbar.hideFilters": "Hide filters",
        "board.toolbar.shortcuts": "Shortcuts",
        "board.toolbar.shortcutsHelp": "Show keyboard shortcuts",
        "board.shortcuts.search": "Focus search",
        "board.shortcuts.newTicket": "Open new ticket",
        "board.filter.allStates": "All states",
        "board.filter.allAssignees": "All assignees",
        "board.filter.allPriorities": "All priorities",
        "board.filter.allTypes": "All types",
        "board.filter.blockedOnly": "Blocked only",
        "board.filter.selectPreset": "Select preset...",
        "board.filter.presetName": "Preset name",
        "board.filter.loadingPresets": "Loading presets...",
        "board.preset.save": "Save",
        "board.preset.rename": "Rename",
        "board.preset.delete": "Delete",
        "board.preset.share": "Share",
        "board.preset.saveView": "Save view",
        "board.preset.openEditor": "Save view",
        "board.preset.editing": "Editing view",
        "board.preset.cancel": "Cancel",
        "board.bulk.moveState": "Move state",
        "board.bulk.assignUser": "Assign user",
        "board.bulk.setPriority": "Set priority",
        "board.bulk.deleteTickets": "Delete tickets",
        "board.bulk.selectState": "Select state...",
        "board.bulk.selectAssignee": "Select assignee...",
        "board.bulk.deleteHint": "Deletes selected tickets.",
        "board.bulk.apply": "Apply bulk action",
        "board.bulk.applying": "Applying...",
        "board.view.loading": "Loading board...",
        "board.view.setup": "Setup",
        "board.view.createWorkflow": "Create your first workflow",
        "board.view.emptyWorkflow":
            "The board is empty because no workflow states exist yet. Initialize a default board to start adding tickets.",
        "board.view.creating": "Creating...",
        "board.view.initialize": "Initialize board",
        "board.view.storySection": "Board stories",
        "board.view.storyHelp":
            "Stories group tickets horizontally across workflow states.",
        "board.view.storiesCount": "{count} stories",
        "board.view.ticketsCount": "{count} tickets",
        "board.view.exitSelect": "Exit select",
        "board.view.selectTickets": "Select tickets",
        "board.view.selectedCount": "{count} selected",
        "board.view.addStory": "Add story",
        "board.view.noMatchingTitle": "No matching tickets",
        "board.view.noMatchingBody": "Nothing matched",
        "board.view.clearFilter": "Clear filter",
        "board.view.story": "Story",
        "board.view.storyGroup": "Story Group",
        "board.view.deleteStory": "Delete story",
        "board.view.storyActions": "Story actions",
        "board.view.storyLabel": "Story",
        "board.view.ticketCount": "{count} tickets",
        "board.view.addTicket": "+ Add ticket",
        "board.view.blocked": "Blocked ({count})",
        "board.view.unassigned": "Unassigned",
        "board.view.dragCard": "Drag card",
        "board.view.ticketId": "ID",
        "board.view.quickActions": "Quick actions",
        "board.view.quickMove": "Move to next state",
        "board.view.quickPriority": "Cycle priority",
        "board.view.quickAssignMe": "Assign to me",
        "board.view.dropHere": "Drop here",
        "board.view.openTicket": "Open ticket",
        "nav.portfolio": "Portfolio",
        "portfolio.title": "Portfolio",
        "portfolio.subtitle": "Cross-project overview · {n} projects",
        "portfolio.export": "Export CSV",
        "portfolio.projects": "Projects",
        "portfolio.search": "Search projects...",
        "portfolio.filter.all": "All health",
        "portfolio.summary.projects": "Projects",
        "portfolio.summary.projectsSub": "{n} at risk",
        "portfolio.summary.open": "Open tickets",
        "portfolio.summary.openSub": "Across all projects",
        "portfolio.summary.blocked": "Blocked",
        "portfolio.summary.blockedSub": "Blocked by dependencies",
        "portfolio.summary.atRisk": "At risk",
        "portfolio.summary.atRiskSub": "With urgent tickets",
        "portfolio.health.critical": "Critical",
        "portfolio.health.warning": "Warning",
        "portfolio.health.healthy": "Healthy",
        "portfolio.open": "open",
        "portfolio.blocked": "blocked",
        "portfolio.urgent": "urgent",
        "portfolio.ends": "ends",
        "portfolio.remaining": "remaining",
        "portfolio.noActiveSprint": "No active sprint",
        "portfolio.goBoard": "Board",
        "portfolio.goStats": "Stats",
        "portfolio.noMatches": "No projects match your filters.",
        "portfolio.clearFilters": "Clear filters",
        "portfolio.filter.allGroups": "All groups",
        "portfolio.closedThisWeek": "closed this week",
        "portfolio.avgCycleHours": "{n}h avg cycle",
        "portfolio.noProjects": "No projects in your workspace yet.",
        "portfolio.noProjectsSub": "Create a project to get started.",
        "header.board": "Board",
        "header.dashboard": "Dashboard",
        "header.settings": "Settings",
        "header.brand": "Ops Console",
        "header.workspaceDefault": "Ticketing Workspace",
        "header.selectProject": "Select project",
        "header.inbox": "Inbox",
        "header.notifications": "Notifications",
        "header.markAllRead": "Mark all read",
        "header.close": "Close",
        "header.mentions": "Mentions",
        "header.assignments": "Assignments",
        "header.noNotifications": "No notifications.",
        "header.markRead": "Mark read",
        "header.logout": "Logout",
        "header.liveWs": "Live updates via WebSocket",
        "header.livePolling": "Fallback updates via polling",
        "newTicket.title": "New ticket",
        "newTicket.subtitle": "Create a card",
        "newTicket.aiTriage": "AI Triage",
        "newTicket.suggest": "Suggest",
        "newTicket.suggesting": "Suggesting...",
        "newTicket.aiDisabled": "AI triage is disabled for this project.",
        "newTicket.applySummary": "Apply summary",
        "newTicket.applyPriority": "Apply priority",
        "newTicket.applyState": "Apply state",
        "newTicket.applyAssignee": "Apply assignee",
        "newTicket.fieldTitle": "Title",
        "newTicket.fieldDescription": "Description",
        "newTicket.fieldType": "Type",
        "newTicket.fieldPriority": "Priority",
        "newTicket.fieldStory": "Story",
        "newTicket.fieldState": "State",
        "newTicket.fieldAssignee": "Assignee",
        "newTicket.shortSummaryPlaceholder": "Short summary",
        "newTicket.descriptionPlaceholder": "What needs to happen?",
        "newTicket.selectStory": "Select a story",
        "newTicket.assigneeSearchPlaceholder": "Search from group members...",
        "newTicket.noUsersFound": "No users found matching \"{query}\"",
        "newTicket.noGroupMembers": "No group members available",
        "newTicket.create": "Create ticket",
        "ticket.titleLabel": "Ticket",
        "ticket.actionsLabel": "Ticket actions",
        "ticket.fieldTitle": "Title",
        "ticket.fieldDescription": "Description",
        "ticket.describePlaceholder": "Describe the ticket...",
        "ticket.fieldType": "Type",
        "ticket.fieldPriority": "Priority",
        "ticket.fieldState": "State",
        "ticket.deleteTicket": "Delete ticket",
        "ticket.incidentMode": "Incident mode",
        "ticket.severity": "Severity",
        "ticket.commander": "Commander",
        "ticket.impact": "Impact",
        "ticket.select": "Select",
        "ticket.unassigned": "Unassigned",
        "ticket.attachments": "Attachments",
        "ticket.upload": "+ Upload",
        "ticket.uploading": "Uploading...",
        "ticket.noFiles": "No files attached.",
        "ticket.dependencies": "Dependencies",
        "ticket.loading": "Loading...",
        "ticket.selectTicket": "Select ticket",
        "ticket.remove": "Remove",
        "ticket.noDependencies": "No dependencies.",
        "ticket.graphTitle": "Graph (2-hop)",
        "ticket.loadingGraph": "Loading graph...",
        "ticket.graphNodesEdges": "{nodes} nodes · {edges} edges",
        "ticket.openGraph": "Open graph",
        "ticket.activity": "Activity",
        "ticket.incidentTimeline": "Incident timeline",
        "ticket.eventsCount": "{count} events",
        "ticket.expand": "Expand",
        "ticket.collapse": "Collapse",
        "ticket.exportPostmortem": "Export postmortem",
        "ticket.loadingTimeline": "Loading timeline...",
        "ticket.noTimeline": "No incident timeline events yet.",
        "ticket.comments": "Comments",
        "ticket.saving": "Saving...",
        "ticket.noComments": "No comments yet.",
        "ticket.addComment": "Add comment (Markdown)",
        "ticket.commentPlaceholder": "Progress, blockers, notes...",
        "ticket.post": "Post",
        "ticket.posting": "Posting...",
        "ticket.saveChanges": "Save changes",
        "ticket.dependencyGraph": "Dependency graph",
        "ticket.dependencyGraphView": "{key} · 2-hop view",
        "ticket.dependencyGraphAria": "Ticket dependency graph",
        "ticket.noDependencyGraph": "No dependency graph data yet.",
        "ticket.relationBlocks": "blocks",
        "ticket.relationBlockedBy": "blocked_by",
        "ticket.relationRelated": "related",
        "ticket.activityChangedState": "changed state from {old} to {new}",
        "ticket.activityChangedPriority":
            "changed priority from {old} to {new}",
        "ticket.activityAssignedTo": "assigned to {new}",
        "ticket.activityRemovedAssignee": "removed assignee",
        "ticket.activityChangedType": "changed type from {old} to {new}",
        "ticket.activityRenamed": "renamed ticket",
        "ticket.activityChangedIncidentSeverity":
            "changed incident severity from {old} to {new}",
        "ticket.unknownValue": "-",
        "dashboard.empty.title": "No tickets yet",
        "dashboard.empty.body":
            "Create tickets on the board to see project statistics here.",
        "dashboard.summary.total": "Total",
        "dashboard.summary.open": "Open",
        "dashboard.summary.closed": "Closed",
        "dashboard.summary.blockedOpen": "Blocked Open",
        "dashboard.byState": "By State",
        "dashboard.dependencyGraph": "Dependency Graph",
        "dashboard.loadingDependencyGraph": "Loading dependency graph...",
        "dashboard.graphNodesEdges":
            "{nodes} nodes · {edges} edges (up to 2-hop expansion)",
        "dashboard.noDependencies": "No dependencies yet.",
        "dashboard.byPriority": "By Priority",
        "dashboard.byType": "By Type",
        "dashboard.byAssignee": "By Assignee",
        "dashboard.recentActivity": "Recent Activity",
        "dashboard.noActivity": "No activity yet.",
        "dashboard.activity.stateChanged": "changed state to {value}",
        "dashboard.activity.priorityChanged": "changed priority to {value}",
        "dashboard.activity.assignedTo": "assigned to {value}",
        "dashboard.activity.removedAssignee": "removed assignee",
        "dashboard.activity.typeChanged": "changed type to {value}",
        "dashboard.activity.renamed": "renamed ticket",
        "dashboard.activity.on": "on",
        "dashboard.sprintForecast": "Sprint Forecast",
        "dashboard.latestSprint": "Latest sprint",
        "dashboard.iterations": "Iterations",
        "dashboard.recalculate": "Recalculate",
        "dashboard.runningForecast": "Running forecast simulation...",
        "dashboard.committed": "Committed",
        "dashboard.projected": "Projected",
        "dashboard.capacity": "Capacity",
        "dashboard.overCapacity": "Over Capacity",
        "dashboard.confidenceLine":
            "Confidence: {confidence}% · Iterations: {iterations} · Capacity entries: {capacity}",
        "dashboard.noForecast": "No sprint forecast available yet.",
        "settings.title": "Settings",
        "settings.subtitle": "Admin workspace",
        "settings.tab.projects": "Projects",
        "settings.tab.users": "Users",
        "settings.tab.webhooks": "Webhooks",
        "settings.tab.workflow": "Workflow",
        "settings.tab.reporting": "Reporting",
        "settings.tab.sprints": "Sprints",
        "settings.tab.audit": "Audit Log",
        "settings.tab.automation": "Automation",
        "settings.automation.title": "Automation Rules",
        "settings.automation.empty": "No automation rules yet.",
        "settings.automation.addRule": "Add Rule",
        "settings.automation.name": "Rule Name",
        "settings.automation.event": "Trigger Event",
        "settings.automation.enabled": "Enabled",
        "settings.automation.actions": "Actions",
        "settings.automation.addAction": "+ Add Action",
        "settings.automation.removeAction": "Remove",
        "settings.automation.save": "Save",
        "settings.automation.cancel": "Cancel",
        "settings.automation.executions": "Execution History",
        "settings.automation.executionEmpty": "No executions yet.",
        "settings.automation.deleteConfirm": "Delete this rule?",
        "settings.automation.conditions": "Conditions (optional)",
        "settings.automation.condFromState": "From State ID",
        "settings.automation.condToState": "To State ID",
        "settings.audit.title": "Audit Log",
        "settings.audit.empty": "No audit log entries yet.",
        "settings.audit.loadMore": "Load more",
        "settings.audit.col.time": "Time",
        "settings.audit.col.actor": "Actor",
        "settings.audit.col.action": "Action",
        "settings.audit.col.resource": "Resource",
        "settings.sprints.duration": "Default Sprint Duration (days)",
        "settings.sprints.capacity": "Capacity Settings",
        "settings.sprints.scope": "Scope",
        "settings.sprints.label": "Label",
        "settings.sprints.capacityValue": "Capacity",
        "settings.sprints.addRow": "+ Add row",
        "settings.sprints.removeRow": "Remove",
        "settings.sprints.saving": "Saving...",
        "settings.sprints.saved": "Saved",
        "ticket.storyPoints": "Story Points",
        "ticket.timeEstimate": "Time Estimate (min)",
        "ticket.timeTracking": "Time Tracking",
        "ticket.logTime": "Log time",
        "ticket.minutes": "Minutes",
        "ticket.hours": "hours",
        "ticket.totalLogged": "Total logged",
        "ticket.noTimeEntries": "No time entries yet.",
        "ticket.description": "Description",
        "newTicket.storyPoints": "Story Points",
        "story.storyPoints": "Story Points (budget)",
        "story.pointsBudget": "SP budget",
        "story.modal.label": "New story",
        "story.modal.heading": "Create a story",
        "story.modal.titleLabel": "Title",
        "story.modal.titlePlaceholder": "Story title",
        "story.modal.descriptionLabel": "Description",
        "story.modal.descriptionPlaceholder": "Shared goal or summary",
        "story.modal.creating": "Creating...",
        "story.modal.create": "Create story",
        "board.view.overBudget": "Over budget",
        "dashboard.configureSettings": "Configure in Settings",
        "ticket.sprints": "Sprints",
        "ticket.addToSprint": "Add to sprint",
        "ticket.removeFromSprint": "Remove",
        "ticket.noSprints": "Not in any sprint.",
        "ticket.selectSprint": "Select sprint…",
        "sprint.sidebar.title": "Sprints",
        "sprint.sidebar.toggle": "Sprints",
        "sprint.sidebar.active": "Active",
        "sprint.sidebar.planned": "Planned",
        "sprint.sidebar.completed": "Completed",
        "sprint.sidebar.progress": "Progress",
        "sprint.sidebar.tickets": "tickets",
        "sprint.sidebar.startSprint": "Start Sprint",
        "sprint.sidebar.completeSprint": "Complete Sprint",
        "sprint.sidebar.noSprints": "No sprints yet.",
        "sprint.complete.title": "Complete Sprint",
        "sprint.complete.movePrompt": "Move incomplete tickets to the next sprint?",
        "sprint.complete.nextSprint": "Next sprint",
        "sprint.complete.noNextSprint": "No planned sprint available for rollover.",
        "sprint.complete.selectAll": "Select all",
        "sprint.complete.deselectAll": "Deselect all",
        "sprint.complete.allDone": "All tickets are done!",
    },
    de: {
        "common.clear": "Leeren",
        "common.loading": "Lädt...",
        "common.close": "Schließen",
        "common.cancel": "Abbrechen",
        "common.delete": "Löschen",
        "common.save": "Speichern",
        "common.add": "Hinzufügen",
        "board.search.placeholder": "Tickets filtern...",
        "board.toolbar.views": "Ansichten",
        "board.toolbar.filters": "Filter",
        "board.toolbar.hideFilters": "Filter ausblenden",
        "board.toolbar.shortcuts": "Shortcuts",
        "board.toolbar.shortcutsHelp": "Tastaturkürzel anzeigen",
        "board.shortcuts.search": "Suche fokussieren",
        "board.shortcuts.newTicket": "Neues Ticket öffnen",
        "board.filter.allStates": "Alle Status",
        "board.filter.allAssignees": "Alle Zuständigen",
        "board.filter.allPriorities": "Alle Prioritäten",
        "board.filter.allTypes": "Alle Typen",
        "board.filter.blockedOnly": "Nur blockierte",
        "board.filter.selectPreset": "Preset wählen...",
        "board.filter.presetName": "Preset-Name",
        "board.filter.loadingPresets": "Presets laden...",
        "board.preset.save": "Speichern",
        "board.preset.rename": "Umbenennen",
        "board.preset.delete": "Löschen",
        "board.preset.share": "Teilen",
        "board.preset.saveView": "Ansicht speichern",
        "board.preset.openEditor": "Ansicht speichern",
        "board.preset.editing": "Ansicht bearbeiten",
        "board.preset.cancel": "Abbrechen",
        "board.bulk.moveState": "Status ändern",
        "board.bulk.assignUser": "Zuweisen",
        "board.bulk.setPriority": "Priorität setzen",
        "board.bulk.deleteTickets": "Tickets löschen",
        "board.bulk.selectState": "Status wählen...",
        "board.bulk.selectAssignee": "Zuständige wählen...",
        "board.bulk.deleteHint": "Löscht ausgewählte Tickets.",
        "board.bulk.apply": "Sammelaktion anwenden",
        "board.bulk.applying": "Wird angewendet...",
        "board.view.loading": "Board wird geladen...",
        "board.view.setup": "Einrichtung",
        "board.view.createWorkflow": "Ersten Workflow erstellen",
        "board.view.emptyWorkflow":
            "Das Board ist leer, weil noch keine Workflow-Status existieren. Initialisiere ein Standard-Board, um Tickets anzulegen.",
        "board.view.creating": "Wird erstellt...",
        "board.view.initialize": "Board initialisieren",
        "board.view.storySection": "Board Stories",
        "board.view.storyHelp":
            "Stories gruppieren Tickets horizontal über Workflow-Status.",
        "board.view.storiesCount": "{count} Stories",
        "board.view.ticketsCount": "{count} Tickets",
        "board.view.exitSelect": "Auswahl beenden",
        "board.view.selectTickets": "Tickets auswählen",
        "board.view.selectedCount": "{count} ausgewählt",
        "board.view.addStory": "Story hinzufügen",
        "board.view.noMatchingTitle": "Keine passenden Tickets",
        "board.view.noMatchingBody": "Nichts gefunden für",
        "board.view.clearFilter": "Filter leeren",
        "board.view.story": "Story",
        "board.view.storyGroup": "Story-Gruppe",
        "board.view.deleteStory": "Story löschen",
        "board.view.storyActions": "Story-Aktionen",
        "board.view.storyLabel": "Story",
        "board.view.ticketCount": "{count} Tickets",
        "board.view.addTicket": "+ Ticket hinzufügen",
        "board.view.blocked": "Blockiert ({count})",
        "board.view.unassigned": "Nicht zugewiesen",
        "board.view.dragCard": "Karte ziehen",
        "board.view.ticketId": "ID",
        "board.view.quickActions": "Schnellaktionen",
        "board.view.quickMove": "In nächsten Status verschieben",
        "board.view.quickPriority": "Priorität wechseln",
        "board.view.quickAssignMe": "Mir zuweisen",
        "board.view.dropHere": "Hier ablegen",
        "board.view.openTicket": "Ticket öffnen",
        "nav.portfolio": "Portfolio",
        "portfolio.title": "Portfolio",
        "portfolio.subtitle": "Projektüberblick · {n} Projekte",
        "portfolio.export": "CSV exportieren",
        "portfolio.projects": "Projekte",
        "portfolio.search": "Projekte suchen...",
        "portfolio.filter.all": "Alle Status",
        "portfolio.summary.projects": "Projekte",
        "portfolio.summary.projectsSub": "{n} gefährdet",
        "portfolio.summary.open": "Offene Tickets",
        "portfolio.summary.openSub": "In allen Projekten",
        "portfolio.summary.blocked": "Blockiert",
        "portfolio.summary.blockedSub": "Durch Abhängigkeiten blockiert",
        "portfolio.summary.atRisk": "Gefährdet",
        "portfolio.summary.atRiskSub": "Mit dringenden Tickets",
        "portfolio.health.critical": "Kritisch",
        "portfolio.health.warning": "Warnung",
        "portfolio.health.healthy": "Gesund",
        "portfolio.open": "offen",
        "portfolio.blocked": "blockiert",
        "portfolio.urgent": "dringend",
        "portfolio.ends": "endet",
        "portfolio.remaining": "übrig",
        "portfolio.noActiveSprint": "Kein aktiver Sprint",
        "portfolio.goBoard": "Board",
        "portfolio.goStats": "Statistik",
        "portfolio.noMatches": "Keine Projekte entsprechen den Filtern.",
        "portfolio.clearFilters": "Filter zurücksetzen",
        "portfolio.filter.allGroups": "Alle Gruppen",
        "portfolio.closedThisWeek": "diese Woche abgeschlossen",
        "portfolio.avgCycleHours": "{n}h Ø Durchlaufzeit",
        "portfolio.noProjects": "Noch keine Projekte im Arbeitsbereich.",
        "portfolio.noProjectsSub": "Erstellen Sie ein Projekt, um zu beginnen.",
        "header.board": "Board",
        "header.dashboard": "Uebersicht",
        "header.settings": "Einstellungen",
        "header.brand": "Ops-Konsole",
        "header.workspaceDefault": "Ticketing-Arbeitsbereich",
        "header.selectProject": "Projekt wählen",
        "header.inbox": "Inbox",
        "header.notifications": "Benachrichtigungen",
        "header.markAllRead": "Alle gelesen",
        "header.close": "Schließen",
        "header.mentions": "Erwähnungen",
        "header.assignments": "Zuweisungen",
        "header.noNotifications": "Keine Benachrichtigungen.",
        "header.markRead": "Als gelesen",
        "header.logout": "Abmelden",
        "header.liveWs": "Live-Updates per WebSocket",
        "header.livePolling": "Fallback-Updates per Polling",
        "newTicket.title": "Neues Ticket",
        "newTicket.subtitle": "Karte erstellen",
        "newTicket.aiTriage": "AI-Triage",
        "newTicket.suggest": "Vorschlagen",
        "newTicket.suggesting": "Wird vorgeschlagen...",
        "newTicket.aiDisabled": "AI-Triage ist für dieses Projekt deaktiviert.",
        "newTicket.applySummary": "Zusammenfassung anwenden",
        "newTicket.applyPriority": "Priorität anwenden",
        "newTicket.applyState": "Status anwenden",
        "newTicket.applyAssignee": "Zuständige anwenden",
        "newTicket.fieldTitle": "Titel",
        "newTicket.fieldDescription": "Beschreibung",
        "newTicket.fieldType": "Typ",
        "newTicket.fieldPriority": "Priorität",
        "newTicket.fieldStory": "Story",
        "newTicket.fieldState": "Status",
        "newTicket.fieldAssignee": "Zuständige",
        "newTicket.shortSummaryPlaceholder": "Kurze Zusammenfassung",
        "newTicket.descriptionPlaceholder": "Was muss passieren?",
        "newTicket.selectStory": "Story auswählen",
        "newTicket.assigneeSearchPlaceholder": "In Gruppenmitgliedern suchen...",
        "newTicket.noUsersFound": "Keine Nutzer gefunden für \"{query}\"",
        "newTicket.noGroupMembers": "Keine Gruppenmitglieder verfügbar",
        "newTicket.create": "Ticket erstellen",
        "ticket.titleLabel": "Ticket",
        "ticket.actionsLabel": "Ticket-Aktionen",
        "ticket.fieldTitle": "Titel",
        "ticket.fieldDescription": "Beschreibung",
        "ticket.describePlaceholder": "Ticket beschreiben...",
        "ticket.fieldType": "Typ",
        "ticket.fieldPriority": "Priorität",
        "ticket.fieldState": "Status",
        "ticket.deleteTicket": "Ticket löschen",
        "ticket.incidentMode": "Incident-Modus",
        "ticket.severity": "Schweregrad",
        "ticket.commander": "Incident Commander",
        "ticket.impact": "Auswirkung",
        "ticket.select": "Auswählen",
        "ticket.unassigned": "Nicht zugewiesen",
        "ticket.attachments": "Anhänge",
        "ticket.upload": "+ Hochladen",
        "ticket.uploading": "Wird hochgeladen...",
        "ticket.noFiles": "Keine Dateien angehängt.",
        "ticket.dependencies": "Abhängigkeiten",
        "ticket.loading": "Lädt...",
        "ticket.selectTicket": "Ticket auswählen",
        "ticket.remove": "Entfernen",
        "ticket.noDependencies": "Keine Abhängigkeiten.",
        "ticket.graphTitle": "Graph (2-Hop)",
        "ticket.loadingGraph": "Graph wird geladen...",
        "ticket.graphNodesEdges": "{nodes} Knoten · {edges} Kanten",
        "ticket.openGraph": "Graph öffnen",
        "ticket.activity": "Aktivität",
        "ticket.incidentTimeline": "Incident-Zeitachse",
        "ticket.eventsCount": "{count} Ereignisse",
        "ticket.expand": "Erweitern",
        "ticket.collapse": "Einklappen",
        "ticket.exportPostmortem": "Postmortem exportieren",
        "ticket.loadingTimeline": "Zeitachse wird geladen...",
        "ticket.noTimeline": "Noch keine Incident-Ereignisse.",
        "ticket.comments": "Kommentare",
        "ticket.saving": "Wird gespeichert...",
        "ticket.noComments": "Noch keine Kommentare.",
        "ticket.addComment": "Kommentar hinzufügen (Markdown)",
        "ticket.commentPlaceholder": "Fortschritt, Blocker, Notizen...",
        "ticket.post": "Posten",
        "ticket.posting": "Wird gepostet...",
        "ticket.saveChanges": "Änderungen speichern",
        "ticket.dependencyGraph": "Abhängigkeitsgraph",
        "ticket.dependencyGraphView": "{key} · 2-Hop-Ansicht",
        "ticket.dependencyGraphAria": "Ticket-Abhängigkeitsgraph",
        "ticket.noDependencyGraph": "Noch keine Graph-Daten.",
        "ticket.relationBlocks": "blockiert",
        "ticket.relationBlockedBy": "blockiert durch",
        "ticket.relationRelated": "verwandt",
        "ticket.activityChangedState": "Status geändert von {old} zu {new}",
        "ticket.activityChangedPriority":
            "Priorität geändert von {old} zu {new}",
        "ticket.activityAssignedTo": "zugewiesen an {new}",
        "ticket.activityRemovedAssignee": "Zuweisung entfernt",
        "ticket.activityChangedType": "Typ geändert von {old} zu {new}",
        "ticket.activityRenamed": "Ticket umbenannt",
        "ticket.activityChangedIncidentSeverity":
            "Incident-Schweregrad geändert von {old} zu {new}",
        "ticket.unknownValue": "-",
        "dashboard.empty.title": "Noch keine Tickets",
        "dashboard.empty.body":
            "Erstelle Tickets im Board, um hier Projektstatistiken zu sehen.",
        "dashboard.summary.total": "Gesamt",
        "dashboard.summary.open": "Offen",
        "dashboard.summary.closed": "Geschlossen",
        "dashboard.summary.blockedOpen": "Offen blockiert",
        "dashboard.byState": "Nach Status",
        "dashboard.dependencyGraph": "Abhängigkeitsgraph",
        "dashboard.loadingDependencyGraph":
            "Abhängigkeitsgraph wird geladen...",
        "dashboard.graphNodesEdges":
            "{nodes} Knoten · {edges} Kanten (bis zu 2-Hop-Erweiterung)",
        "dashboard.noDependencies": "Noch keine Abhängigkeiten.",
        "dashboard.byPriority": "Nach Priorität",
        "dashboard.byType": "Nach Typ",
        "dashboard.byAssignee": "Nach Zuständigen",
        "dashboard.recentActivity": "Letzte Aktivitäten",
        "dashboard.noActivity": "Noch keine Aktivitäten.",
        "dashboard.activity.stateChanged": "Status geändert zu {value}",
        "dashboard.activity.priorityChanged":
            "Priorität geändert zu {value}",
        "dashboard.activity.assignedTo": "zugewiesen an {value}",
        "dashboard.activity.removedAssignee": "Zuweisung entfernt",
        "dashboard.activity.typeChanged": "Typ geändert zu {value}",
        "dashboard.activity.renamed": "Ticket umbenannt",
        "dashboard.activity.on": "bei",
        "dashboard.sprintForecast": "Sprint-Prognose",
        "dashboard.latestSprint": "Neuester Sprint",
        "dashboard.iterations": "Iterationen",
        "dashboard.recalculate": "Neu berechnen",
        "dashboard.runningForecast":
            "Prognose-Simulation wird ausgeführt...",
        "dashboard.committed": "Geplant",
        "dashboard.projected": "Prognose",
        "dashboard.capacity": "Kapazität",
        "dashboard.overCapacity": "Überkapazität",
        "dashboard.confidenceLine":
            "Konfidenz: {confidence}% · Iterationen: {iterations} · Kapazitätseinträge: {capacity}",
        "dashboard.noForecast": "Noch keine Sprint-Prognose verfügbar.",
        "settings.title": "Einstellungen",
        "settings.subtitle": "Admin-Arbeitsbereich",
        "settings.tab.projects": "Projekte",
        "settings.tab.users": "Benutzer",
        "settings.tab.webhooks": "Webhooks",
        "settings.tab.workflow": "Workflow",
        "settings.tab.reporting": "Reporting",
        "settings.tab.sprints": "Sprints",
        "settings.tab.audit": "Audit-Log",
        "settings.tab.automation": "Automatisierung",
        "settings.automation.title": "Automatisierungsregeln",
        "settings.automation.empty": "Noch keine Regeln.",
        "settings.automation.addRule": "Regel hinzufügen",
        "settings.automation.name": "Regelname",
        "settings.automation.event": "Auslöserevent",
        "settings.automation.enabled": "Aktiv",
        "settings.automation.actions": "Aktionen",
        "settings.automation.addAction": "+ Aktion hinzufügen",
        "settings.automation.removeAction": "Entfernen",
        "settings.automation.save": "Speichern",
        "settings.automation.cancel": "Abbrechen",
        "settings.automation.executions": "Ausführungshistorie",
        "settings.automation.executionEmpty": "Noch keine Ausführungen.",
        "settings.automation.deleteConfirm": "Diese Regel löschen?",
        "settings.automation.conditions": "Bedingungen (optional)",
        "settings.automation.condFromState": "Von Status-ID",
        "settings.automation.condToState": "Zu Status-ID",
        "settings.audit.title": "Audit-Log",
        "settings.audit.empty": "Noch keine Audit-Log-Einträge.",
        "settings.audit.loadMore": "Mehr laden",
        "settings.audit.col.time": "Zeit",
        "settings.audit.col.actor": "Akteur",
        "settings.audit.col.action": "Aktion",
        "settings.audit.col.resource": "Ressource",
        "settings.sprints.duration": "Standard-Sprint-Dauer (Tage)",
        "settings.sprints.capacity": "Kapazitätseinstellungen",
        "settings.sprints.scope": "Bereich",
        "settings.sprints.label": "Bezeichnung",
        "settings.sprints.capacityValue": "Kapazität",
        "settings.sprints.addRow": "+ Zeile hinzufügen",
        "settings.sprints.removeRow": "Entfernen",
        "settings.sprints.saving": "Wird gespeichert...",
        "settings.sprints.saved": "Gespeichert",
        "ticket.storyPoints": "Story Points",
        "ticket.timeEstimate": "Zeitschätzung (min)",
        "ticket.timeTracking": "Zeiterfassung",
        "ticket.logTime": "Zeit erfassen",
        "ticket.minutes": "Minuten",
        "ticket.hours": "Stunden",
        "ticket.totalLogged": "Gesamt erfasst",
        "ticket.noTimeEntries": "Noch keine Zeiteinträge.",
        "ticket.description": "Beschreibung",
        "newTicket.storyPoints": "Story Points",
        "story.storyPoints": "Story Points (Budget)",
        "story.pointsBudget": "SP-Budget",
        "story.modal.label": "Neue Story",
        "story.modal.heading": "Story erstellen",
        "story.modal.titleLabel": "Titel",
        "story.modal.titlePlaceholder": "Story-Titel",
        "story.modal.descriptionLabel": "Beschreibung",
        "story.modal.descriptionPlaceholder": "Gemeinsames Ziel oder Zusammenfassung",
        "story.modal.creating": "Erstelle...",
        "story.modal.create": "Story erstellen",
        "board.view.overBudget": "Über Budget",
        "dashboard.configureSettings": "In Einstellungen konfigurieren",
        "ticket.sprints": "Sprints",
        "ticket.addToSprint": "Zu Sprint hinzufügen",
        "ticket.removeFromSprint": "Entfernen",
        "ticket.noSprints": "In keinem Sprint.",
        "ticket.selectSprint": "Sprint wählen…",
        "sprint.sidebar.title": "Sprints",
        "sprint.sidebar.toggle": "Sprints",
        "sprint.sidebar.active": "Aktiv",
        "sprint.sidebar.planned": "Geplant",
        "sprint.sidebar.completed": "Abgeschlossen",
        "sprint.sidebar.progress": "Fortschritt",
        "sprint.sidebar.tickets": "Tickets",
        "sprint.sidebar.startSprint": "Sprint starten",
        "sprint.sidebar.completeSprint": "Sprint abschließen",
        "sprint.sidebar.noSprints": "Noch keine Sprints.",
        "sprint.complete.title": "Sprint abschließen",
        "sprint.complete.movePrompt": "Unvollständige Tickets in den nächsten Sprint verschieben?",
        "sprint.complete.nextSprint": "Nächster Sprint",
        "sprint.complete.noNextSprint": "Kein geplanter Sprint für die Übernahme verfügbar.",
        "sprint.complete.selectAll": "Alle auswählen",
        "sprint.complete.deselectAll": "Alle abwählen",
        "sprint.complete.allDone": "Alle Tickets sind erledigt!",
    },
};

const detectLocale = (): Locale => {
    if (typeof window === "undefined") return "en";
    const saved = window.localStorage.getItem(STORAGE_KEY);
    if (saved === "en" || saved === "de") return saved;
    const browser = window.navigator.language.toLowerCase();
    return browser.startsWith("de") ? "de" : "en";
};

const activeLocale = ref<Locale>(detectLocale());

if (typeof document !== "undefined") {
    document.documentElement.lang = activeLocale.value;
}

const interpolate = (
    text: string,
    params?: Record<string, string | number>,
): string => {
    if (!params) return text;
    return Object.entries(params).reduce(
        (result, [key, value]) =>
            result.split(`{${key}}`).join(String(value)),
        text,
    );
};

export const useI18n = () => {
    const setLocale = (locale: Locale) => {
        activeLocale.value = locale;
        if (typeof document !== "undefined") {
            document.documentElement.lang = locale;
        }
        if (typeof window !== "undefined") {
            window.localStorage.setItem(STORAGE_KEY, locale);
        }
    };

    const t = (key: string, params?: Record<string, string | number>) => {
        const dict = messages[activeLocale.value] || messages.en;
        const template = dict[key] || messages.en[key] || key;
        return interpolate(template, params);
    };

    return {
        availableLocales: ["en", "de"] as const,
        locale: computed(() => activeLocale.value),
        setLocale,
        t,
    };
};
