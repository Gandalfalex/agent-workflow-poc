...
<template>
  <div class="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 py-20 lg:flex-row">
    <div class="flex-1 space-y-4">
      ...
      <p class="text-sm text-muted-foreground">
        Use your Keycloak credentials to access the live board and webhooks.
      </p>
      <div
    ...
    <div class="w-full max-w-md rounded-3xl border border-border bg-card/80 p-6 shadow-sm">
      <div class="space-y-2">
        <p class="text-xs uppercase tracking-[0.3em] text-muted-foreground">
          Welcome back
        </p>
      ...
      <form class="mt-6 space-y-4" @submit.prevent="onSubmit">
        ...
        <div v-if="props.isChecking" class="text-xs text-muted-foreground">
          Checking session...
        </div>
        <Button
          class="w-full"
          type="submit"
          :disabled="!props.canLogin || props.loginBusy || props.isChecking"
        >
          {{ props.loginBusy || props.isChecking ? "Signing in..." : "Sign in" }}
        </Button>
      </form>
    </div>
  </div>
